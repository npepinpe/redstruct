  ...
  module Types
    class SortedSet < Redstruct::Types::Struct
      ...
      def count(lower: nil, upper: nil)
        return self.connection.zcount(@key, parse_score_bound(lower, '-'), parse_score_bound(upper, '+'))
      end
      ...
      def to_enum(match:, count:)
        return self.connection.zscan_each(match: @factory.isolate(match), count: count)
      end

      def parse_bound(bound, default) # :nodoc:
        case bound
        when nil
          default
        when -Float::INFINITY
